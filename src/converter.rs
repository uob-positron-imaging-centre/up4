...
pub fn csv_multi_converter(
 ...
 in the same time frame. Be careful interpreting the data!"
    ...
    if !(mode == "chain") {
        unimplemented!("This mode is not implemented yet.\n Currently only chained mode is implemented, meaning that the particles are ");
    };
    ...
    let particle_data: Vec<ndarray::Array2<f64>> = {
        ...
        for idx in 0..particles {
            ...
            if interpolate {
                temp_data = convertertools::interpolate(temp_data);
            }
            if vel {
                if columns.len() >= 5 {
                    panic!(
