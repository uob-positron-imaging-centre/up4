...
/// Converter for modern, xml-based vtk files
#[derive(Default)]
pub struct XMLVTKConverter {
...
impl XMLVTKConverter {
    ...
    /// Convert a folder with modern, xml-based vtk files into a HDF5 file
    ///
    ///
    /// # Examples
    ///
    ...
    ///     .add_id_field("id")
    ///     .add_radius_field("radius")
    ///     .add_type_field("type");
    /// converter.write_hdf5_from_folder(
    ///     "folder",
    ///     1e-5,
    ///     "output.hdf5",
    ///     r"vtk_(\d+).vtu",
    ...
    ///     .add_radius_field("radius")
    ///     .add_type_field("type");
    /// converter.write_hdf5_from_files(
    ///     vec!["file1.vtk", "file2.vtk", "file3.vtk"],
    ///     1e-5,
    ///     "output.hdf5",
    ///     r"vtk_(\d+).vtu",
    /// );
...
/// Converter for modern, ascii-based vtk files

#[derive(Default)]
pub struct LegacyVTKConverter {
    velocity_field_name: Option<String>,
    id_field_name: Option<String>,
    radius_field_name: Option<String>,
    type_field_name: Option<String>,
...
impl LegacyVTKConverter {
    ...
    /// Convert a folder with legacy vtk files into a HDF5 file
    ///
    ///
    /// # Examples
    ///
    ...
    pub fn write_hdf5_from_files(
        ...
        for (file_id, filename) in filenames.iter().enumerate() {
            ...
            print_debug!("Parsing particle IDs");
            let particle_ids = if let Some(ref id_field) = self.id_field_name {
                legacytools::get_field::<u64>(filename, id_field)
            } else {
                panic!("No ID field specified");
            };
            let sort_list = make_sortlist(&particle_ids);
            ...
            let particle_radius = if let Some(ref radius_field) = self.radius_field_name {
                sort_by_array(
                    legacytools::get_field::<f64>(filename, radius_field),
                    &sort_list,
                )
            } else if let Some(ref diameter_field) = self.diameter_field_name {
                sort_by_array(
                    legacytools::get_field::<f64>(filename, diameter_field)
                        .iter()
                        .map(|x| x * 0.5)
                        .collect::<Vec<f64>>(),
                    &sort_list,
            ...
            let particle_type = if let Some(ref type_field) = self.type_field_name {
                sort_by_array(
                    legacytools::get_field::<u64>(filename, type_field),
                    &sort_list,
                )
            } else {
                print_warning!("No type field specified, defaulting to 1");
            ...
            let particle_velocity = if let Some(ref velocity_field) = self.velocity_field_name {
                sort_by_array(
                    legacytools::get_field::<f64>(filename, velocity_field),
                    &sort_list,
                )
            } else {
                panic!("No velocity field specified");
            ...
            print_debug!("  Creating position dataset");
            let particle_positions =
                sort_by_array(legacytools::get_positions::<f64>(filename), &sort_list);
            let particle_positions =
                Array::from_shape_vec((particle_positions.len() / 3, 3), particle_positions)
                    .unwrap();
            print_debug!("New: {:?}", particle_positions);
