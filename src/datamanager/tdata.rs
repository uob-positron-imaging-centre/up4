...
use pyo3::prelude::*;
use super::{Timestep,GlobalStats,DataManager, Manager};
...
impl TData{
    pub fn new(
        ...
        print_debug!("TData: Generating new instance with file: {}\
        ...
        print_debug!("TData: Generation complete. First buffer update starting, buffer size: {}",
        data.buffer.len());
    ...
    fn read_single(&mut self,timestep:usize)->&Timestep{
        ...
        let position = self.file.group(&format!("timestep {}",timestep))
        ...
        let velocity = self.file.group(&format!("timestep {}",timestep))
                        .expect(&format!("Can not find group \"timestep {}\" in file {}",
                            timestep,
                            self.file.filename()))
                        .dataset("velocity")
        ...
        //Radius
        let radius = self.file.group(&format!("timestep {}",timestep))
            .expect(&format!("Can not find timestep {} in file {}",timestep, self.file.filename()))
            .dataset("radius")
            .expect(&format!(
            ...
            .to_owned();
        let density =  match self.file.group(&format!("timestep {}",timestep))
            .expect(&format!("Can not find timestep {} in file {}",timestep, self.file.filename()))
            .dataset("density")
            {
                Ok(s)=>{
                    ...
                    .to_owned()},
                Err(_)=>{
                    print_warning!("Can not find dataset \"density\" in file {} \
                    \n using ones instead ", self.file.filename());
        ...
        let clouds = self.file.group(&format!("timestep {}",timestep))
            .expect(&format!("Can not find timestep {} in file {}",timestep, self.file.filename()))
            .dataset("ppcloud")
            .expect(&format!(
        ...
        let time = self.file.group(&format!("timestep {}",timestep))
            .expect(&format!("Can not fine timestep {} in file {}",timestep, self.file.filename()))
            .dataset("time")
            .expect(&format!(
            ...
            .to_owned();
        let particleid = self.file.group(&format!("timestep {}",timestep))
            .expect(&format!("Can not find timestep {} in file {}",timestep, self.file.filename()))
            .dataset("particleid")
            .expect(&format!(
...
impl DataManager for TData {
...

    {
        ...
        print_debug!("TData: Checking requested timestep {} in range {:?}", timestep, self.range);
