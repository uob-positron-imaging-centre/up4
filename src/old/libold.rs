...
mod functions;
mod datamanager;
use crate::datamanager::DataManager as dm;
//mod datamanager{
//    pub mod datamanager;
//}
...
fn upppp_rust(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_wrapped(wrap_pyfunction!(jumps_pept))?;
    ...
    m.add_wrapped(wrap_pyfunction!(stats))?;
    Ok(())
}
...
fn stats<'py>(
    ...
    let manager = dm::init(filename, 10);
    let time = manager.get_attr("time");
    let version = manager.fileversion;
    println!("HDF5-file based on the H5Part datastructure for particle data.");
    println!("Version: {}",version);
    let type_ = &manager.filetype;
    println!("Filetype: {:?}",type_);
    println!("The min and max time of this set is : {:?}",time);
    let part_num =manager.get_attr("particle number")[0];
    println!("Number of Particles: {}",part_num);
    let mean_vel = manager.get_attr("velocity magnitude")[1];
    println!("Mean velocity of: {} m/s",mean_vel);
    let min_vel = manager.get_attr("velocity magnitude")[0];
    let max_vel = manager.get_attr("velocity magnitude")[2];
    println!("Minimum velocity {} m/s \nMaximum Velocity {} m/s",min_vel,max_vel);
...
functions::jumps_pept(filename, min_velocity, min_distance)
