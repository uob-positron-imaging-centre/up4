...
use plotly::common::{ColorBar, ColorScaleElement, Fill, Line, Marker, Mode};
use plotly::cone::Anchor;
use plotly::heat_map::Smoothing;
use plotly::{
    layout::{AxisConstrain, Layout},
    HeatMap, Plot, Scatter, Trace,
};
...
impl VectorPlotter {
    ...
    pub fn create_quiver_traces(
        ...
        for (x_line, y_line, x_head, y_head, c) in
            izip!(barb_x, barb_y, arrow_x, arrow_y, colour_vector)
        {
    ...
    pub fn create_unit_vector_background(
        &self,
        layout: Layout,
        square: bool,
        ...
        if square {
            let mut axes_iter = axes.into_iter();
            let x_axis: plotly::layout::Axis = axes_iter
                .next()
                .unwrap()
                .unwrap_or(plotly::layout::Axis::new());
            let y_axis: plotly::layout::Axis = axes_iter
                .next()
                .unwrap()
                .unwrap_or(plotly::layout::Axis::new());
            ...
            let layout = layout
                .y_axis(
                    y_axis.anchor("x").range(vec![
                        *self.ydata.min_skipnan() - dy_start * 0.5,
                        *self.ydata.max_skipnan() + dy_end * 0.5,
                    ]), //.scale_ratio(1.)
                )
                .x_axis(x_axis.constrain(AxisConstrain::Domain).range(vec![
                    *self.xdata.min_skipnan() - dx_start * 0.5,
                    *self.xdata.max_skipnan() + dx_end * 0.5,
                ]));
