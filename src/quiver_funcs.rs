...
pub fn gen_quiver_arrows(
    ...
    let arrow_len: Array1<f64> = izip!(&data.vdata, &data.udata)
        .map(|(u, v)| f64::hypot(*u, *v))
        .collect::<Array1<f64>>()
        * ARROW_SCALE;

    // get barb angles
    let barb_ang: Array1<f64> = izip!(&data.vdata, &data.udata)
...
pub fn plot_arrows(data: ArrowData, n: f64) {
    ...
    for (x_line, y_line, x_head, y_head) in izip!(barb_x, barb_y, arrow_x, arrow_y) {
        ...
        let xpl = vec![
            x_line.0, x_line.1, x_head.0, //x_head.1,
            x_head.2, x_head.3,
        ];

        let ypl = vec![
            y_line.0, y_line.1, y_head.0, //y_head.1,
            y_head.2, y_head.3,
        ];
